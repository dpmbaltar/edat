...
public class ArbolBinInt {
	...
	private int calcularAltura(NodoArbolInt nodo, int nivel, int altura) {
		NodoArbolInt nodoIzquierdo = nodo.getIzquierdo(),
	...
	private int calcularNivel(int elemento,
							int nivelElemento,
							int nivelActual,
							NodoArbolInt nodo) {
		if (nodo != null) {
			if (nodo.getElemento() == elemento) {
				...
				nivelElemento = calcularNivel(elemento,
											nivelElemento,
											(nivelActual + 1),
											nodo.getIzquierdo());
				if (nivelElemento == -1) {
	...
	private NodoArbolInt buscarPadre(int elemento,
								NodoArbolInt padre,
								NodoArbolInt hijo) {
		...
		if (hijo != null) {
			if (hijo.getElemento() == elemento) {
				...
				if (padreBuscado == null) {
					padreBuscado = buscarPadre(elemento, hijo, hijo.getDerecho());
				}
			}
		}
	...
	private void preorden(NodoArbolInt nodo, ListaInt lista) {
		NodoArbolInt izquierdo = nodo.getIzquierdo(),
